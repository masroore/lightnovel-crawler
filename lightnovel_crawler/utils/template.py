...
def template(*args, **kwargs):
    ...
    if not TEMPLATES[tplid]:
        raise FileNotFoundError('Template ({}) not found'.format(tpl))
    for dictarg in args[1:]:
        kwargs.update(dictarg)
    return TEMPLATES[tplid].render(kwargs)


def render(tpl, *args, **vars):
    if isinstance(tpl, str):
        tpl = SimpleTemplate(tpl)
    return tpl.render(*args, **vars)


if __name__ == '__main__':
    s = template('start {{var}} end', var='var')
    print(s)
    ...
    s = template('stpl_simple', var='var')
    print(s)
...
---end'''
    ...
    s = render(t, content='1234')
    print(s)

    s = template('stpl_t2main', content='1234')
    print(s)
